...
function loadCustomCaptcha($height = 46, $width = '100%x', $bgcolor = '#003', $textcolor = '#abc')
{
...
function levelCommission($user, $amount, $commissionType, $trx, $setting){
    $meUser = $user;
    ...
    while ($i < $level) {
        ...
        $transactions[] = [
            ...
            'charge' => 0,
            'trx_type' => '+',
            'details' => 'level '.$i.' Referral ('.ucfirst($commissionType).') Commission From ' . $user->username,
            'trx' => $trx,
            'created_at'=>now()
        ];
            ...
            <div class="card mb-3">
                <div class="card-body">
                    <div class="row align-items-center">
                        ...
                        <div class="col">
                            <p class="mb-0"><b>Name:</b> '.$data->firstname.' '.$data->lastname.'</p>
                            <p class="mb-0"><b>Username:</b> '.$data->username.'</p>
                            <p class="mb-0"><b>Joined:</b> '.showDateTime($data->created_at).'</p>
                            <p class="mb-0"><b>1st Deposit:</b> '.$fd.' '.$general->cur_text.'</p>
                            <p class="mb-0"><b>Up Leader:</b> '.$sp.'</p>
                        </div>
                    </div>
                </div>
            </div>
...
function get_user_vip_level($user_id){
    ...
    if ( !blank($user->vip_level) ){
        $level = $user->vip_level;
    }
    
    $deposits =  Deposit::where('user_id', $user_id)->where('status', 1)->sum('final_amo');
    $refers = User::where('ref_by', $user_id)->count();
    $deposits =  getAmount($deposits);
...
function user_notification_count(){
     $c = Notification::where('user_id', auth()->user()->id)->where('status', 0)->count();
     if ( $c > 0){
