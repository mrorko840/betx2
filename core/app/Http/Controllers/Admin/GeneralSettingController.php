...
class GeneralSettingController extends Controller
{
    ...
    {
        // dd($request->d_and_w_preset);
        $request->validate([
            'base_color' => 'nullable', 'regex:/^[a-f0-9]{6}$/i',
            'timezone' => 'required',
            'min_limit' => 'required|numeric|gt:0|regex:/^\d+(\.\d{1,2})?$/',
            'max_limit' => 'required|numeric|gt:min_limit|regex:/^\d+(\.\d{1,2})?$/',
        ],[
            'min_limit.regex' => 'Max .99 is allowed as fractional number',
            'max_limit.regex' => 'Max .99 is allowed as fractional number',
        ]);
        ...
        $general->min_limit = $request->min_limit;
        $general->max_limit = $request->max_limit;
        $general->d_and_w_preset = $request->d_and_w_preset;
        $general->save();
